...
def predict_case(data, args):
    ...
    mm = data['mm_f'] * np.where(data['mm_f'] == 0, 0, 1) + \
         get_gaussian(data['m_d_matrix']) * np.where(data['mm_f'] == 1, 0, 1)
    dd = data['dd_s'] * np.where(data['dd_s'] == 0, 0, 1) + \
         get_gaussian(data['m_d_matrix'].t()) * np.where(data['dd_s'] == 1, 0, 1)
...
def get_data(args):
    ...
    data['miRNA_number'] = mf.shape[0]
    data['disease_number'] = dss.shape[0]
