...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # ✅ 添加这一行
...
def nx_to_PyGGraph(g, graph_label, node_labels, node_features, max_node_label, class_values):
    y = torch.tensor([graph_label], dtype=torch.float).to(device)
    ...
    edge_index = torch.tensor([i + j, j + i], dtype=torch.long).to(device)

    x_struct = torch.tensor(one_hot(node_labels, max_node_label + 1), dtype=torch.float).to(device)
    x_feat = torch.tensor(node_features, dtype=torch.float).to(device)
    x = torch.cat([x_struct, x_feat], dim=1)
...
def subgraph_extraction_labeling(ind, A, h=1, u_features=None, v_features=None, class_values=None):
    ...
    node_features = None  # 添加兜底初始化，避免未定义
