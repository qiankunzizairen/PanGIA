...
def evaluate(model, loader, flag):
    ...
    if flag == 2:
        one_pred_result = np.vstack((predictions.cpu().numpy(), labels.cpu().numpy()))

    fpr, tpr, _ = metrics.roc_curve(labels.cpu(), predictions.cpu(), pos_label=1)
    auc = metrics.auc(fpr, tpr)

    return auc if flag == 1 else (auc, one_pred_result)
