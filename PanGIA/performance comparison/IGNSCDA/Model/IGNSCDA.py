'''
The original code is from NGCF
...
class IGNSCDA(object):
    def __init__(self, data_config, pretrain_data):
        # argument settings
        self.model_type = 'IGNSCDA'
        ...
        *********************************************************
        ...
        if self.alg_type in ['ignscda']:
            self.ua_embeddings, self.ia_embeddings = self._create_ignscda_embed()
        ...
        *********************************************************
        ...
        """
        self.u_g_embeddings = tf.nn.embedding_lookup(self.ua_embeddings, self.users, name='u_g_embeddings')
        self.pos_i_g_embeddings = tf.nn.embedding_lookup(self.ia_embeddings, self.pos_items, name='pos_i_g_embeddings')
        self.neg_i_g_embeddings = tf.nn.embedding_lookup(self.ia_embeddings, self.neg_items, name='neg_i_g_embeddings')
    ...
    def _create_ignscda_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            all_embeddings += [norm_embeddings] # 把原始embedding和经过了一层之后的embedding连接在一起
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    model = IGNSCDA(data_config=config, pretrain_data=pretrain_data)
    ...
    *********************************************************
    ...
    ua_embeddings = graph.get_tensor_by_name('split:0')
    ia_embeddings = graph.get_tensor_by_name('split:1')
    user_feature, item_feature = sess.run([ua_embeddings, ia_embeddings], feed_dict={
        model.node_dropout: eval(args.node_dropout),
        model.mess_dropout: eval(args.mess_dropout)
    })
    # # 将这两个feature保存起来
    # with h5py.File('./circRNA_disease_gcn_embedding_32_feature_file/circRNA_disease_gcn_embedding_128_feature_fold5.h5') as hf:
    #     hf['user_feature'] = user_feature
    #     hf['item_feature'] = item_feature

    # 将这两个feature保存起来
    with h5py.File('./circRNA_disease_gcn_embedding_32_origin_feature_file/circRNA_disease_gcn_embedding_32_origin_feature_fold5.h5', 'w') as hf:
        hf['user_feature'] = user_feature
        hf['item_feature'] = item_feature

    # with h5py.File('./case_study_embedding_feature_file/case_study_embedding_feature.h5') as hf:
    #     hf['user_feature'] = user_feature
    #     hf['item_feature'] = item_feature
