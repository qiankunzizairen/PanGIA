...
class HANEncoder(nn.Module):
    ...
    Args (keyword-only):
        rna_in_dim:   feature dim of RNA nodes
        d_feat_dim:   feature dim of disease nodes
        hidden_dim:   hidden dim per head
        num_heads:    number of attention heads
        dropout:      dropout probability
        metadata:     hetero-graph metadata for HANConv
    """
    def __init__(
        self,
        *,
        ...
        super().__init__()
        # 1) map disease features to same dim as RNA
        self.lin_d = nn.Linear(d_feat_dim, rna_in_dim)

        # 2) HANConv: PyG HANConv outputs 'out_channels' regardless of heads
        self.han = HANConv(
...
class MMoEHANNet(nn.Module):
    """
    Multi-task MMoE + HAN model for ncRNA–disease prediction.

    Args (keyword-only):
        metadata:     hetero-graph metadata
        rna_in_dim:   input feature dim of RNA nodes
        d_feat_dim:   input feature dim of disease nodes
        hidden_dim:   hidden dim per HAN head
        expert_dim:   output dim of each expert
        num_experts:  number of experts
        num_heads:    number of attention heads
        num_tasks:    number of tasks (4)
        dropout:      dropout probability
    """
    def __init__(
        self,
        *,
        ...
        # 4) task-specific gating: map hidden_dim -> num_experts
        self.gates = nn.ModuleDict({
            rel: nn.Linear(hidden_dim, num_experts, bias=False)
            for rel in ['mi-d','circ-d','lnc-d','pi-d']
        })
        # 5) task heads: combine expert + cross into expert_dim
        ...
        # 6) cross-task attention on T tasks
        ...
        # 7) project disease reps to expert_dim
